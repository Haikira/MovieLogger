...
## Decision
Use ASP.NET Core Web API for the presentation/HTTP layer, implemented in the `MovieLogger.Api` project (`Microsoft.NET.Sdk.Web`). Controllers under `server\src\MovieLogger.Api\Controllers\` (e.g. `MoviesController.cs`) expose the API surface, with `Program.cs` wiring up services and middleware. Swagger/OpenAPI is enabled for interactive API documentation, as noted in the repo README.
