...
## Decision
Define specialized repository interfaces that extend the generic one — `IMovieRepository : IRepository<Movie>` and `IGenreRepository : IRepository<Genre>` (`MovieLogger.Service\Repositories\`) — and implement them as `MovieRepository : Repository<Movie>, IMovieRepository` and `GenreRepository : Repository<Genre>, IGenreRepository` (`MovieLogger.DAL\Repositories\`). These override generic methods where needed (e.g. `MovieRepository` overrides `GetAllAsync`/`GetByIdAsync` to `.Include(m => m.Genres)`) and add entity-specific methods (e.g. `GenreRepository.GetByIdsAsync`). Each is registered in DI alongside the generic registration.
